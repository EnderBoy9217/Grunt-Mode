...
global table<entity, int> placedTurrets
...
void function DeployableTurrentWeapon_Init()
{
		...
		AddCallback_OnPlayerKilled( ResetTurrets )
	#endif
...
var function OnWeaponPrimaryAttack_turretweapon( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	#if SERVER
		int turretCount = placedTurrets[ownerPlayer]
		if (turretCount != 2)
		{
...
entity function DeployTurret( entity player, vector origin, vector angles, entity weapon, SentryTurretPlacementInfo placementInfo )
{
	...
	else if( weapon.HasMod( "burnmeter_ap_turret_weapon" ) || weapon.HasMod( "burnmeter_ap_turret_weapon_inf" ) || weapon.HasMod( "sentry_tech_grunt_mode" ))
